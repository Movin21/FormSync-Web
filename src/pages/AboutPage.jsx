...
import { TEAM_MEMBERS, SUPERVISORS } from '../data/teamData';
...
/* ─── Supervisor Card ───────────────────────────────────────────── */
const SupervisorCard = ({ supervisor, index }) => {
  const photoUrl = `${import.meta.env.BASE_URL}assets/${encodeURIComponent(supervisor.photo)}`;
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-brand-100 bg-white shadow-card overflow-hidden"
    >
      ...
      <section className="py-10 border-t border-neutral-100">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-neutral-900">Research <span className="gradient-text">Supervisors</span></h2>
          </div>
          <div className="flex flex-wrap justify-center gap-6">
            {SUPERVISORS.map((supervisor, i) => (
              <div key={supervisor.email} className="w-full max-w-sm">
                <SupervisorCard supervisor={supervisor} index={i} />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-10 border-t border-neutral-100 bg-neutral-50/50">
        <div className="mx-auto max-w-3xl px-5 sm:px-8 text-center">
          ...
          >
            ...
            <p className="text-sm text-neutral-500 mt-2">Faculty of Computing : Software Engineering</p>
            <p className="text-sm text-neutral-500">Academic Year 2025 / 2026</p>
            <div className="mt-4">
              <a
